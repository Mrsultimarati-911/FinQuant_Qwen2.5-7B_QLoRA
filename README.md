...
### 1. 技术架构
* **硬件基础**：因特尔XEON-6530 + RTX-4090 + 60G内存 + 100G硬盘
* **环境配置**：Ubuntu 22.04.5 + CUDA 12.9 + Python 3.11 + Pytorch 2.6.0
* **量化策略**：采用 `4-bit` 量化加载基座模型，将显存基础占用压缩至 5GB 极小值。
* **显存释放**：开启 `Gradient Checkpointing`（梯度检查点）；采用 `paged_adamw_32bit` 分页优化器防止显存峰值溢出。
* **参数调优**：为平衡收敛速度与显存约束，采用 `Batch Size = 1` 配合 `Gradient Accumulation = 16`，等效大批次稳健训练；设定核心超参 LoRA `Rank = 16`, `Alpha = 32`。

### 2. 数据工程 
* **知识提取**：以约300份金工深度研报为原料进行 PDF 解析、利用Python的rapidocr_pdf库中的RapidOCRPDF函数进行 OCR 切块与清洗，剥离冗余噪音。
* **格式对齐**：调用大模型 API 批量构造出超一万条高质量 `Instruction-Output` 对，严格剔除带有“AI 机器味”的泛泛之谈，行文风格深度对齐资深量化研究员的叙事品味。
